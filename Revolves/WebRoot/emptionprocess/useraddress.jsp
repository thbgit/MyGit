...
<html>
	...
	<head>
		...
		<script type="text/javascript">
			$(function() {
				$('.address1 label').click(function() {
				var label = $('.address1 label');
					var labelLength = $('.address1 label').length;
					for(var i = 0; i < labelLength; i++) {
						if(this == $('.address1 label').get(i)) {
							$('.address1 label').eq(i).addClass('red');
							
						} else {
							$('.address1 label').eq(i).removeClass('red');
						}
					}
				});
			});
...
-->
			...
			<div class="links">
				<ul class="links_list">
					...
					<li>
						<a href="selectAll.do">会员尊享</a>
					</li>
				</ul>
			</div>

			<div class="main">
				<form action="payorder.do" method="post" onsubmit="return choiceaddr()">
				<div id="cart2left">
					<p>收货地址</p>
					<div class="address1">
					<c:forEach items="${addrList }" var="userAddr">
						<label>
							<input type="radio" name="addr" id="choiceaddress" value="${userAddr.addrid }">
							<span>${userAddr.addr1 }</span><br>
							<span>${userAddr.addr2 }&nbsp;&nbsp;${userAddr.addr3 }</span><br>
							<span>${userAddr.addr4 }</span><br>
							<span>${userAddr.code }</span><br>
							<span>${userAddr.telnum }</span><br>
						</label>
					</c:forEach>
					</div>
					<a class="myddress" href="mycore/unindext.jsp">管理收货地址<img src="${pageContext.request.contextPath }/img/flow/confirmation2.png"/></a>
						<input class="Address" type="submit" value="保持并继续" />
				</div>
			</form>
				<div class="right">
